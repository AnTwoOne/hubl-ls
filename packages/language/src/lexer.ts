...
export const TOKEN_TYPES = Object.freeze({
  Text: "Text", // The text between HubL statements or expressions
...
export function tokenize(
  ...
  main: while (cursorPosition < source.length) {
    // First, consume all text that is outside of a HubL statement or expression
    if (codeDepth === 0 || insideRaw) {
      ...
      if (insideRaw) {
        ...
        while (
          cursorPosition < source.length &&
          // Keep going until we hit the next HubL statement or expression
          !(
    ...
    // Consume and ignore all whitespace inside HubL statements or expressions
